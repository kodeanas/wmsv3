...
const qcdSalesRows: QcdSalesRow[] = [
  {
    code_document: "QCD-SALE-001",
    buyer: "CV Kualitas Prima",
    bag: "Bag QCD Grade A",
    harga_asal: 3760000,
    grand_total: 3490000,
  },
  {
    code_document: "QCD-SALE-002",
    buyer: "PT Standar Mulia",
    bag: "Bag QCD Grade B",
    harga_asal: 2985000,
    grand_total: 2775000,
  },
  {
    code_document: "QCD-SALE-003",
    buyer: "Toko Karya Bersama",
    bag: "Bag QCD Mix",
    harga_asal: 2860000,
    grand_total: 2590000,
  },
...
export default function QcdSalesPage() {
  const [selectedDetail, setSelectedDetail] = useState<QcdSalesRow | null>(null);
  const [isDetailModalOpen, setIsDetailModalOpen] = useState(false);

  const handleOpenDetail = (row: QcdSalesRow) => {
    setSelectedDetail(row);
    setIsDetailModalOpen(true);
  };

  const handleExportBag = (row: QcdSalesRow) => {
    const content = [
      "Detail Penjualan QCD",
    ...
    link.href = url;
    link.download = `detail-bag-qcd-${row.code_document}.txt`;
    link.click();
  ...
  return (
    <div className="space-y-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-bold text-slate-800">Penjualan QCD</h2>
...
<section className="grid grid-cols-1 gap-5 md:grid-cols-2">
      ...
      <section>
        <DataTable
          title="Data Penjualan QCD"
          data={qcdSalesRows}
          exportConfig={{ show: true }}
          topButton={{ label: "Buat Penjualan", href: "/qcd-sales/form-sales" }}
          columns={[
            {
              key: "code_document",
