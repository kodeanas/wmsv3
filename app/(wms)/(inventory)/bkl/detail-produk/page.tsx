...
const bundlingStickerData = [
	{
		barcode: "STK-88001",
		nama: "Sticker Big Feed",
		qty: 40,
		sticker: "Big",
		harga_asal: 75000,
		harga_gudang: 77000,
...
type BundlingStickerItem = (typeof bundlingStickerData)[number];

const renderStickerBadge = (sticker: string) => {
...
const getStickerInfo = (price: number) => {
	if (price < 50000) {
		return { type: "Tiny", colorName: "Hijau", dotClass: "bg-emerald-400" };
	}
	if (price < 80000) {
		return { type: "Small", colorName: "Kuning", dotClass: "bg-amber-400" };
	}
	return { type: "Big", colorName: "Merah", dotClass: "bg-red-400" };
};
...
export default function DetailProdukStickerPage() {
	const [selectedHistory, setSelectedHistory] = useState<HistoryItem | null>(
		null,
	);
	const [selectedBundlingProduct, setSelectedBundlingProduct] =
		useState<BundlingStickerItem | null>(null);
	...
	const stickerInfo = getStickerInfo(barangAsal.harga);
...
<section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
					...
					<div className="flex items-center gap-6 pr-2">
						...
						<div className="flex flex-col items-end">
							...
							<div className="flex items-center gap-1.5">
								<div className={`h-2.5 w-2.5 rounded-full ${stickerInfo.dotClass}`} />
								<span className="text-sm font-extrabold text-slate-700">
									{stickerInfo.colorName}
								</span>
							</div>
						</div>
					</div>
				...
				<Tabs
					...
					items={[
						...
						},
						{
							...
							content: (
								<DataTable
									title="Data Produk Sticker"
									data={bundlingStickerData}
									columns={[
										{ key: "barcode", header: "Barcode", accessor: "barcode" },
										{ key: "nama", header: "Nama", accessor: "nama" },
										...
										},
										{
											...
											header: "Harga Gudang",
											accessor: (item: any) =>
												`Rp ${item.harga_gudang.toLocaleString("id-ID")}`,
										},
									]}
									searchableKeys={["barcode", "nama", "sticker"]}
									actions={{
										customActions: [
											{
												label: "Detail",
												icon: <Eye size={18} />,
												color: "blue",
												onClick: (item) => setSelectedBundlingProduct(item),
											},
										],
									}}
									queryParamPrefix="bundling"
								/>
							),
						},
					]}
			...
			<Modal
				isOpen={Boolean(selectedBundlingProduct)}
				onClose={() => setSelectedBundlingProduct(null)}
				title="Informasi Produk Sticker"
				size="md"
				footer={
					<Button
						label="Tutup"
						variant="outline"
						onClick={() => setSelectedBundlingProduct(null)}
					/>
				}
			>
				{selectedBundlingProduct && (
					<div className="grid grid-cols-1 gap-4 rounded-xl border border-slate-100 bg-slate-50 p-4 md:grid-cols-2 md:gap-x-6">
						<InfoDisplay label="Barcode" value={selectedBundlingProduct.barcode} />
						<InfoDisplay label="Nama" value={selectedBundlingProduct.nama} />
						<InfoDisplay label="Qty" value={selectedBundlingProduct.qty} />
						<InfoDisplay label="Sticker" value={selectedBundlingProduct.sticker} />
						<InfoDisplay
							label="Harga Asal"
							value={`Rp ${selectedBundlingProduct.harga_asal.toLocaleString("id-ID")}`}
						/>
						<InfoDisplay
							label="Harga Gudang"
							value={`Rp ${selectedBundlingProduct.harga_gudang.toLocaleString("id-ID")}`}
						/>
					</div>
				)}
			</Modal>
